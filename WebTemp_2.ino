...
Serial.print(humid);
Serial.println(" %");
Serial.print("Presión A. : ");
float pat = (bme.readPressure()/ 100.0F);
Serial.print(pat);
Serial.println(" hPa");

// Sincronizacion reloj NTP
NTPDate();
displayCurrentDate();
// displayCurrentTime();
...
client.println("<head>\n<meta charset='UTF-8'>");
client.println("<title>BME280 Sensor</title>");
...
client.println("<H2>BME280 Sensor</H2>");
...
client.println(" %");
client.print("Presión At. : ");
client.print(pat, 2);
client.println(" hPa");
client.print("Actualizado : ");
client.print(hour());
client.print(":");
client.print(minute());
client.print(":");
...
//get a random server from the pool
  ...
  if (!cb) {
    Serial.println("Sin respuesta NTP.");
            }
  else {
    ...
    unsigned long ajuste = epoch;
    epoch = secsSince1900 - seventyYears;
    
    // print Unix time:
    //Serial.println(epoch);
    // print the hour, minute and second:
    Serial.print("Ajuste hora NTP: ");       // UTC is the time at Greenwich Meridian (GMT)
    Serial.println((epoch - ajuste)% 60);
    ...
    setTime(epoch); // Ajusta el reloj con el tiempo NTP
    adjustTime(3600*2); // Añade 2 horas a UTC
